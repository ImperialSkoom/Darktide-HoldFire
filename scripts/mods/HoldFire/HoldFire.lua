...
local function current_debug_ray_origin()
	...
	local ray_origin = player_smart_targeting_extension:_targeting_parameters()
...
local function nearby_destructible_target(hit_position, ignored_unit)
	...
	local ray_origin, forward = player_smart_targeting_extension and player_smart_targeting_extension:_targeting_parameters()
...
local function resolve_destructible_target(target_unit, target_position, allow_nearby_fallback)
...
local function raycasted_destructible_target()
	...
	for i = 1, num_hits do
		...
		if hit_position and hit_unit and hit_unit ~= player_unit then
			local resolved_target = resolve_destructible_target(hit_unit, hit_position, false)
	...
	local resolved_target = resolve_destructible_target(fallback_hit_unit, fallback_hit_position, true)
...
local function retained_enemy_target_active(current_frame, horizontal_tolerance, enemy_vertical_tolerance)
	...
	local retained_target_data = update_precision_target(ENEMY_RETENTION_TARGETING_TEMPLATE)
...
local function hovered_priority_target()
	...
	local target_data = update_precision_target(ENEMY_SMART_TARGETING_TEMPLATE)
	...
	if is_target_alive(target_unit) and breed_data then
		...
		if retained_enemy_target_active(current_frame, horizontal_tolerance, enemy_vertical_tolerance) then
		...
		elseif not setting_value("target_destructibles") then
			...
			local object_target_data = update_precision_target(OBJECT_SMART_TARGETING_TEMPLATE)
			...
			local resolved_object_target = resolve_destructible_target(object_target_unit, object_target_position)
			...
			if not cached_priority_target then
				raycast_result, raycast_debug = raycasted_destructible_target()
...
if CLASS.SkitariusOmnissiah then
